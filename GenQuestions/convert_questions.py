...
# Constants
DATASETS = {
    'dev': 'data/squad/dev-v2.0.json',
    'sample1k': 'out/none_n1000_k1_s0.json',
    'train': 'data/squad/train-v2.0.json',
    'toy': 'data/squad/splited/toy.json',
    '0': 'data/squad/splited/0.json',
    '1': 'data/squad/splited/1.json',
    '2': 'data/squad/splited/2.json',
    '3': 'data/squad/splited/3.json',
    '4': 'data/squad/splited/4.json',
    '5': 'data/squad/splited/5.json',
    '6': 'data/squad/splited/6.json',
    '7': 'data/squad/splited/7.json',
    '8': 'data/squad/splited/8.json',
    '9': 'data/squad/splited/9.json',
}
CORENLP_CACHES = {
    'dev': 'data/squad/corenlp_cache.json',
    'sample1k': 'data/squad/corenlp_cache.json',
    'train': 'data/squad/train_corenlp_cache.json',
    'toy': 'data/squad/splited/toy_corenlp_cache.json',
    '0': 'data/squad/splited/0_corenlp_cache.json',
    '1': 'data/squad/splited/1_corenlp_cache.json',
    '2': 'data/squad/splited/2_corenlp_cache.json',
    '3': 'data/squad/splited/3_corenlp_cache.json',
    '4': 'data/squad/splited/4_corenlp_cache.json',
    '5': 'data/squad/splited/5_corenlp_cache.json',
    '6': 'data/squad/splited/6_corenlp_cache.json',
    '7': 'data/squad/splited/7_corenlp_cache.json',
    '8': 'data/squad/splited/8_corenlp_cache.json',
    '9': 'data/squad/splited/9_corenlp_cache.json',
}
...
COMMANDS = ['print-questions', 'print-answers', 'corenlp', 'convert-q',
            ...
            'dump-placeholder', 'dump-lies', 'dump-highConf', 'dump-hcSeparate', 'dump-altAll', 'nlpcc']
            ...
            if new_words:
                for nw in new_words:
                    ...
                    if strategy.endswith('separate'):
                        ...
                        found = True
                        # print('ori: ', t['word'], ' new: ', new_tok['word'])
                        break
    ...
    for t in tokens:
        if t['pos'] == 'CD' or t['word'].isdigit():
            try:
                ...
                value = 10  # fallback
            if value > 50:
                new_val = str(value - 25)  # Year
            else:  # Day of month
                if value > 15:
                    new_val = str(value - 11)
                else:
                    new_val = str(value + 11)
        else:
...
def gen_unanswerable_questions(dataset):
    corenlp_cache = load_cache()
    nearby_word_dict = load_nearby_words()
    postag_dict = load_postag_dict()
    out_data = []
    ...
    for article in dataset['data']:
        ...
        for paragraph in article['paragraphs']:
            ...
            new_qas = []
            for qa in paragraph['qas']:
                # only generate unanswerable questions from previously answerable ones
                if not qa['is_impossible']:
                    ...
                    q_list = alter_question(
                        ...
                        postag_dict, strategy='high-conf')
                    new_qa = {}
                    new_qa['question'] = q_list[0][0]
                    new_qa['id'] = qa['id'] + '_adv'
                    new_qa['answers'] = []
                    new_qa['is_impossible'] = True
                    new_qas.append(new_qa)
            paragraph['qas'].extend(new_qas)
    with open(os.path.join('NLPCC', OPTS.dataset + '.json'), 'w') as f:
...
def main():
    ...
    elif OPTS.command == 'nlpcc':
        gen_unanswerable_questions(dataset)
    else:
